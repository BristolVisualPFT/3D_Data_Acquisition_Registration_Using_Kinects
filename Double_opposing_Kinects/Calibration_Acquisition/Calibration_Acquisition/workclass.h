...
public:
	...
	static bool writeIR()
	{
		...
		if(bufferIRSave.try_pop(tmp))
		{

			sprintf(m_cpIRfilepathTmp,"%s\\infraRed%s%d.png",
					cpFullDirectPathIR,
					(m_iIRWriteCnt<10)?"0000":(m_iIRWriteCnt<100)?"000":(m_iIRWriteCnt<1000)?"00":(m_iIRWriteCnt<10000)?"0":"",
					m_iIRWriteCnt);
				imwrite(m_cpIRfilepathTmp,*(tmp->matrix));
		
			//Now write in metadata file
			char myDate [12];
			sprintf(myDate,"%.2hd%.2hd%.2hd%.3hd",tmp->st.wHour,tmp->st.wMinute,tmp->st.wSecond,tmp->st.wMilliseconds);
			fprintf(m_fpIRMeta,"%s, %d, %lld, %g\n", myDate, m_iIRWriteCnt, tmp->i64DeviceTimeStamp, (tmp->tbbtcNativeTimeStamp - m_tcStart).seconds());
	...
	static bool myPushDepthSave(Mat *depthImSave, INT64 ts, tbb::tick_count nts, SYSTEMTIME system_t)
	{
	...
	static bool myPushRGBSave(Mat *rgbImSave, INT64 ts, tbb::tick_count nts, SYSTEMTIME system_t)
	{
		ImageStruct * tmp = new ImageStruct();
	...
	static bool myPushSkelSaveScreen(Joint joints[], INT64 ts, tbb::tick_count nts, cv::Point m_2dpScreenCoords[JointType_Count], SYSTEMTIME system_t)
	{
	...
	static bool myPushSkelSave(Joint joints[], INT64 ts, tbb::tick_count nts, SYSTEMTIME system_t)
	{
		Skeleton * tmp = new Skeleton();
	...
	static bool myPushIRDisplay(Mat *irImDisplay)
	{
		Mat * tmp = new Mat(irImDisplay->rows,irImDisplay->cols, CV_8UC4);
		int dataSize=irImDisplay->rows*irImDisplay->cols*((int)irImDisplay->elemSize());

		memcpy(tmp->data,irImDisplay->data,  (size_t)dataSize);

		bufferIRDisplay.push(tmp);
		return true;
	}

	static bool myPushIRSave(Mat *irImSave, INT64 ts, tbb::tick_count nts, SYSTEMTIME system_t)
	{
		ImageStruct * tmp = new ImageStruct();

		tmp->matrix = new Mat(irImSave->rows,irImSave->cols, CV_16U);
		int dataSize=irImSave->rows*irImSave->cols*((int)irImSave->elemSize());

		memcpy(tmp->matrix->data,irImSave->data,  (size_t)dataSize);

		tmp->i64DeviceTimeStamp = ts;
		tmp->tbbtcNativeTimeStamp = nts;
		tmp->st = system_t;

		bufferIRSave.push(tmp);
	...
	{		
		...
		{
			CreateIRFolder();
			sprintf(cpTmpPath,"%s\\infraRedMeta.txt",cpFullDirectPath);
			m_fpIRMeta = fopen(cpTmpPath,"a");
	...
	static void  InitStuff()
	{
    	...
    	if(m_bViewForce)
		{						
			imgIRDisplay = new Mat(cIRHeight, cIRWidth, CV_8UC4);
			namedWindow( "InfraRed window", WINDOW_AUTOSIZE );
			moveWindow("InfraRed window", 400, 10);
		}
		if(m_bViewForce || m_bSaveForce)
		{
			m_pIRRGBX = new RGBQUAD[cIRWidth * cIRHeight];	
			imgIR = new Mat(cIRHeight,cIRWidth, CV_16U);
		}
	...
	{		
		...
		if(m_bSaveForce)
		{
			tbb::tbb_thread pMyThread3 = tbb::tbb_thread( writeIRwrap);
		}
		if(m_bSaveBody)
		{
	...
	static void  mainLoop(int iNumIts, int numSaves)
	{
		...
		tbb::tbb_thread pMyThread4 = tbb::tbb_thread( UpdateIRWrap);
		...
		while(m_bRunMainLoop)
		{
			...
			if(m_bViewForce && bufferIRDisplay.try_pop(tmp))
			{
				imshow( "InfraRed window", *tmp);
				waitKey(2);
				tmp->release();
			}
		...
		{
			Sleep(100);
			if(bufferDepthSave.unsafe_size() > 0 || bufferRGBSave.unsafe_size() > 0 || bufferIRSave.unsafe_size() > 0 || bufferSkelSave.unsafe_size() > 0)
			{
				continue;
			}
			else
	...
	static void ProcessBodyOriginal(INT64 nTime, int nBodyCount, IBody** ppBodies, SYSTEMTIME system_t)
	{
						...
						if (SUCCEEDED(hr))
						{
							if(m_bSaveBody)
							{
								if(m_bScreenSkeleton)
								{
									...
									myPushSkelSaveScreen(joints, nTime, tbb::tick_count::now(),m_2dpScreenCoords, system_t);

								}
								else
								{
									myPushSkelSave(joints, nTime, tbb::tick_count::now(), system_t);
								}
							}
							if(m_bViewBody)
							{
	...
	static void UpdateBody()
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				ProcessBodyOriginal(nTime, BODY_COUNT, ppBodies, st_tmp);
	...
	static void UpdateDepth()
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				ProcessDepth(nTime, pBuffer, nWidth, nHeight, nDepthMinReliableDistance, nDepthMaxReliableDistance, st_tmp);
			}
	...
	static void UpdateColour()
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if(m_bSaveRGB)
				{
					ProcessColor(nTime, pBuffer, nWidth, nHeight, st_tmp);			
				}
				if(m_bViewRGB)
				{
	...
	static void UpdateIR()
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				ProcessInfrared(nTime, pBuffer, nWidth, nHeight, st_tmp);
			}
	...
	static void ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight, SYSTEMTIME system_t) 
	{

		// Make sure we've received valid data
		if (pBuffer && (nWidth == cColorWidth) && (nHeight == cColorHeight))
		{		
			myPushRGBSave(imgColour, nTime, tbb::tick_count::now(), system_t);
		}
	}


	static void ProcessDepth(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, USHORT nMinDepth, USHORT nMaxDepth, SYSTEMTIME system_t)
	{
		// Make sure we've received valid data
		if (pBuffer && (nWidth == cDepthWidth) && (nHeight == cDepthHeight))
		{
			...
			if(m_bSaveDepth)
			{
				...
				myPushDepthSave(imgDepth, nTime, tbb::tick_count::now(), system_t);
			}
	...
	static void ProcessInfrared(INT64 nTime, const UINT16* pBuffer, int nWidth, int nHeight, SYSTEMTIME system_t)
	{
		// Make sure we've received valid data
		if (pBuffer && (nWidth == cIRWidth) && (nHeight == cIRHeight))
		{
			...
			{
				//GetFilenamePNG();		
				imgIR->data = (uchar*)pBuffer;
				myPushIRSave(imgIR, nTime, tbb::tick_count::now(), system_t);
			}

			if(m_pIRRGBX && m_bViewForce)
			{
				RGBQUAD* pDest = m_pIRRGBX;
				...
				imgIRDisplay->data = (uchar*) m_pIRRGBX;
     			myPushIRDisplay(imgIRDisplay);
	...
	static void CreateRGBFolder()
	{
		...
		static void CreateIRFolder()
	{
		sprintf(cpFullDirectPathIR,"%s\\infraRed",cpFullDirectPath);
		wchar_t wtext[1000];
		mbstowcs(wtext, cpFullDirectPathIR, strlen(cpFullDirectPathIR)+1);//Plus null
		LPWSTR ptr = wtext;
