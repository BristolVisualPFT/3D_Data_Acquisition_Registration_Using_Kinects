...
static bool writeSkeleton()
	...
	static void  mainLoop(int iNumIts, int numSaves)
	{
		...
		tbb::tbb_thread pMyThread6 = tbb::tbb_thread( UpdateSocketReceiveWrap);
		...
		while(m_bRunMainLoop)
		{
			if(m_bMarknSpirePoint)
			{
				char cpTmpPath[1000];
				GetFolderNameSpiro();
				sprintf(cpTmpPath,"%s\\nSpireFrame.txt",cpFullDirectPath);
				FILE * fp = fopen(cpTmpPath,"w");
				fprintf(fp,"RGB_frameNo: %d, Depth_frameNo: %d, Body_No: %d\n",m_iColourWriteCnt,m_iDepthWriteCnt,m_iBodyWriteCnt);
				fprintf(fp,"%s\n%s",m_cpStartSPiroTime,m_cpTestType);
				fclose(fp);
				m_bMarknSpirePoint = false;
			}
...
static void UpdateIR()
	...
	static void UpdateSocketReceive()
	{
		...
		if (iResult == 1){
			if (recvbuf[0] == 'R')
				RGB_receive_flag = true;
			else if (recvbuf[0] == 'D')
				Depth_receive_flag = true;
			else if (recvbuf[0] == 'B')
				Body_receive_flag = true;
			else if (recvbuf[0] == 'M'){			
